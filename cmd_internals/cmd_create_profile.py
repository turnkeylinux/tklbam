...
* dirindex: index of timestamps, ownership and permissions for dirindex.conf
            paths
...
Arguments:
    ...
    <conf>          Path to configuration file with list of includes/exclude
                    paths ('-' for stdin input)

Options:

    -f --force      Overwrite non-empty directories

    --no-packages   Don't create a list of installed packages from
                    /var/lib/dpkg/status
    ...
    --no-dirindex   Don't create an index of file timestamps, ownerships and
                    permissions.

                    Without this we won't be able to detect changes since the
                    profile was generated so the backup will include everything
                    wholesale.
                    (e.g., all files in /etc vs only files in /etc that have
                    changed)

    --root=PATH     Use this as the root path, instead of /
                    This is useful for generating backup profiles for chroot
                    filesystems


Usage examples:

    # create my-custom-profile by profiling state of files in the paths in
    # profile.conf
    ...
    echo /etc -/etc/.git \
            | tklbam-internal create-profile -- my-custom-profile/ -
...
def usage(e: Optional[str | Error | getopt.GetoptError] = None) -> NoReturn:
    ...
    print(f"Syntax: {sys.argv[0]} [ -options ] output/profile/ <conf>",
          file=stdout)
    ...
    for line in fh.readlines():
        ...
        for path in _paths:
            ...
            if not re.match(r'^-?/', path):
                raise Error(f"{repr(path)} is not an absolute path, try"
                            f" {os.path.abspath(path)} instead")
...
def main():
    ...
    if exists(path_output):
        ...
        if not opt_force and os.listdir(path_output) != []:
            fatal(f"'{path_output}' is not an empty directory (use --force to"
                  " override)")
    ...
    try:
        conf_paths = parse_conf(sys.stdin
                                if path_conf == '-'
                                else open(path_conf))
    except Error as e:
        fatal(e)

    profile = ProfileGenerator(conf_paths, path_output, opt_root,
                               packages=opt_packages, dirindex=opt_dirindex)
    ...
    if exists(profile.paths.dirindex):
        print("\n# Index of file timestamps, ownerships and permissions for"
              " paths in dirindex.conf")
